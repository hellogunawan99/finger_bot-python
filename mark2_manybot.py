...
async def send_telegram_message(bot_token, chat_id, department_name, records, event_type, singapore_tz):
...
async def run_bot(bot_data):
    ...
    while True:
        ...
        for data in bot_data:
            ...
            if latest_records:
                await send_telegram_message(bot_token, chat_id, department_name, latest_records, 'in', singapore_tz)
            ...
            if latest_records:
                await send_telegram_message(bot_token, chat_id, department_name, latest_records, 'out', singapore_tz)

if __name__ == "__main__":
    # Define bot data including bot token, chat ID, department name, and event types
    bot_data = [
        {'bot_token': 'BOT_TOKEN_1', 'chat_id': 'CHAT_ID_1',
            'department_name': 'mine dispatch development'},
        {'bot_token': 'BOT_TOKEN_2', 'chat_id': 'CHAT_ID_2',
            'department_name': 'another department'},
        # Add more bot data as needed
    ]

    asyncio.run(run_bot(bot_data))
